...
impl<'de, A> Visitor<'de> for ProofVisitor<A> {
  ...
  fn visit_str<E>(self, h: &str) -> Result<Self::Value, E>
  where
    E: serde::de::Error,
  {
...
impl Prover {
  ...
  pub fn verify(&self) -> Verifier<slice::Iter<'_, String>> {
    Verifier(self.tape.iter())
  }
}

impl Db for Prover {
  fn auth<A: Serialize>(&mut self, a: A) -> Proof<A> {
    let h = hash(&a);
    Proof {
      value: Some(a),
      hash: h,
...
impl<'at, T> Iterator for Verifier<T>
where
  T: Iterator<Item = &'at String>,
{
  type Item = &'at String;
